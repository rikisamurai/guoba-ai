...
- `apps/docs/` — documentation site
  ...
  - API docs auto-generated by `scripts/build-docs.ts`, driven by a single `lib/packages.ts` table — one row per package. Each row's `layout` (`topical` | `flat`) selects how TypeDoc reads the source and how the postprocess pass reshapes the output. See `apps/docs/CLAUDE.md` for details.
