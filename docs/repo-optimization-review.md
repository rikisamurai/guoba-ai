...
- 缺失字段：`description`、`keywords`、`author`、`license`、`repository`、`homepage`、`bugs`、`sideEffects: false`、`./package.json` 导出
- 改：补齐这些字段；`sideEffects: false` 让消费方可以 tree-shake
...
### P2-1. 补 `useThrottle` trailing-call 与 delay 变化测试
- 文件：`packages/guoba-hook/test/useThrottle.test.ts`
- 加用例：单次 prop 变化后是否触发尾调用；mid-flight 改 `interval` 是否重新计时

### P2-2. 补 `useDebounce` delay 中途变化测试
...
- `set`/`construct`：原型污染（同 P0-2 验证用例）
- `crush`：嵌套数组对象作为叶子的边界
...
### P2-9. 缺 LICENSE / README / CONTRIBUTING / PR 模板
- 改：根目录添加 `LICENSE`（与 package.json `license` 字段一致）、`README.md`（包说明 + badge + 链接到 docs）、`.github/PULL_REQUEST_TEMPLATE.md`、`.github/ISSUE_TEMPLATE/{bug,feature}.yml`
